...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for (neighbor <- b.legalNeighbors ) yield (neighbor._1, neighbor._2 :: history)).to(LazyList)
  /**
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(Set((startBlock, List())).to(LazyList), Set(startBlock))
