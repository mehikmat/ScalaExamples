...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.groupBy(x=>x.toLower) map{case (k,v)=>(k,v.length)}).toList.sortBy(f=>f._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.flatten.mkString)//wordOccurrences(s.reduceLeft((x,y)=>x+y))
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def deriveAnagrams(o:Occurrences):List[Sentence]= {
      if (o.isEmpty) {
        List(Nil)
      } else {
        ...
        for (i <- combs if dictionaryByOccurrences.keySet(i);
             ...
             s <- deriveAnagrams(subtract(o, i))) yield {j :: s}
      }
    }
    deriveAnagrams((sentenceOccurrences(sentence)))
  }

  def main(args:Array[String]){
    // println(combinations(t))
    println(subtract(List(('a',2),('b',2),('c',2),('d',2)),List(('a',2),('d',1))))
  }
}
